...
export function scoreAgainst(oppRef, length, seedStr, playerRef) {
  const opp = resolveOpponent(oppRef);
  ...
  for (let r = 0; r < length; r++) {
    let pm;
    if (playerRef === 'C') pm = 'C';
    else if (playerRef === 'D') pm = 'D';
    else pm = r === 0 ? player.first(pRng) : player.move(foeHist, selfHist, r, pRng);
    pm = pm === 'D' ? 'D' : 'C';
    const omRaw = r === 0 ? opp.first(oRng) : opp.move(selfHist, foeHist, r, oRng);
    const om = omRaw === 'D' ? 'D' : 'C';
    foeHist.push(pm);
    selfHist.push(om);
...
// The reference field: how a handful of textbook strategies would score against
// today's opponent. Shown next to the player as a mini-leaderboard.
const BENCH_STRATS = [
  { id: 'mirror', name: 'Tit for Tat' },
  { id: 'win-stay', name: 'Pavlov' },
  { id: 'grudge', name: 'Grim' },
  { id: 'forgiving', name: 'Generous TfT' },
  { id: 'pushover', name: 'Always Cooperate' },
  { id: 'bully', name: 'Always Defect' },
];

// ---------------------------------------------------------------------------
// The full daily puzzle spec. The UI must not surface the opponent's
// name / blurb until the game is over.
// ---------------------------------------------------------------------------
export function dailyPuzzle(dateStr = todayStr()) {
  ...
  return {
    ...
    oppRef,
    seed: 'daily:' + dateStr,
    bench: BENCH_STRATS.map((s) => ({
      name: s.name,
      score: scoreAgainst(oppRef, length, `bench:${s.id}:${dateStr}`, s.id),
    })),
  };
}
