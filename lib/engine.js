...
// ---------------------------------------------------------------------------
export const ROUND_OPTIONS = [120, 140, 160, 180, 200];
...
export function computeTwist(dateStr) {
  ...
  const rounds = ROUND_OPTIONS[Math.floor(rng() * ROUND_OPTIONS.length)];
  const noise = NOISE_OPTIONS[Math.floor(rng() * NOISE_OPTIONS.length)];
  return { rounds, noisePct: noise.pct, noiseLabel: noise.label };
}
...
export function computeStandings(roster, twist, dateStr, opts = {}) {
  ...
  for (let i = 0; i < n; i++) {
    for (let j = i + 1; j < n; j++) {
      ...
      const res = playMatch(A.strat, B.strat, twist.rounds, twist.noisePct, seed);
      ...
      if (keepMatches) {
        ...
        ta.matches.push({
          ...
          myScore: res.scoreA,
          oppScore: res.scoreB,
          myAvg: res.scoreA / twist.rounds,
          oppAvg: res.scoreB / twist.rounds,
        ...
        tb.matches.push({
          ...
          myScore: res.scoreB,
          oppScore: res.scoreA,
          myAvg: res.scoreB / twist.rounds,
          oppAvg: res.scoreA / twist.rounds,
  ...
  const rows = roster.map((e) => {
    ...
    const avg =
      t.opponents > 0 ? t.points / (twist.rounds * t.opponents) : 0;
    return {
      id: String(e.id),
      name: e.name,
      house: !!e.house,
