...
//
// `field` = every named historical strategy (Axelrod-tournament style) scored
// against today's hidden opponent. Deterministic; no backend. Real players get
// merged into this list when a datastore is connected.
// ---------------------------------------------------------------------------
export function dailyPuzzle(dateStr = todayStr()) {
  ...
  const field = NAMED
    .filter((s) => s.id !== oppId) // don't rank a strategy against itself
    .map((s) => ({
      id: s.id,
      name: s.name,
      nice: s.nice,
      score: scoreAgainst(oppRef, length, `field:${s.id}:${dateStr}`, 'named:' + s.id),
    }))
