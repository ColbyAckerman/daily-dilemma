...
export function computeAllTime(strategies, todayStr, launchDateStr = LAUNCH_DATE) {
  ...
  const acc = new Map();
  const perDay = []; // [{ day, byId: { id: avg } }] — feeds the trend chart
  ...
  for (let d = 0; d < days.length; d++) {
    ...
    const dayEntry = { day, byId: {} };
    perDay.push(dayEntry);
    for (let i = 0; i < standings.length; i++) {
      const row = standings[i];
      if (row.opponents < 1) continue;
      dayEntry.byId[row.id] = row.avg;
      if (!acc.has(row.id)) {
        acc.set(row.id, {
          id: row.id,
          name: row.name,
  ...
  // Trend series for the chart: the current top strategies' daily avg over the
  // window. Only days where a strategy actually played get a point.
  const topIds = rows.slice(0, 6).map((r) => r.id);
  const series = topIds.map((id) => {
    ...
    return {
      ...
      house: row.house,
      points: perDay
        .filter((pd) => pd.byId[id] != null)
        .map((pd) => ({ day: pd.day, avg: pd.byId[id] })),
    };
  });

  return { days: days.length, rows, series, window: perDay.map((p) => p.day) };
}
...
export function simulateDraft(draftDef, strategies, twist, dateStr) {
  ...
  const rankById = new Map(standings.map((r) => [r.id, r.rank]));
  const bouts = (mine ? mine.matches : [])
    .map((m) => ({
      oppId: m.oppId,
      oppName: m.oppName,
      oppHouse: m.oppHouse,
      oppRank: rankById.get(m.oppId) || null,
      rounds: m.rounds,
      myScore: m.myScore,
      oppScore: m.oppScore,
      myAvg: m.myAvg,
      oppAvg: m.oppAvg,
      result: m.result,
      moves: sampleMoves(m.myHist, m.oppHist, 52),
    }))
    // Highest-ranked opponents first (the bosses), so the projected rank
    // climbs back up as the easier bouts land later — a comeback arc.
    .sort(
      (a, b) =>
        (a.oppRank || 999) - (b.oppRank || 999) ||
        (a.oppName < b.oppName ? -1 : 1)
    );
...
function sampleMoves(myHist, oppHist, maxN) {
  const len = myHist.length;
  if (len <= maxN) return myHist.map((m, i) => ({ me: m, opp: oppHist[i] }));
  const step = Math.ceil(len / maxN);
  const out = [];
  for (let i = 0; i < len; i += step) out.push({ me: myHist[i], opp: oppHist[i] });
  return out;
}
